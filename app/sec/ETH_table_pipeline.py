#!/usr/bin/env python3
# sec_eth_timeline_ixbrl.py
# Pull filings (default 8-K) from --from-date onward, parse ETH table from iXBRL/HTML,
# and output a SIMPLE timeline with columns: ticker, date, purchase, total_holdings.
...
# CONFIG
# =========================
USER_AGENT = "Ally Zach <ally@panteracapital.com>"  # REQUIRED: real name + email
...
def candidate_html_urls(files_df: pd.DataFrame, primary_doc: str) -> List[str]:
    ...
    def rank(name: str) -> Tuple[int,int,int]:
        ...
        r1 = 0 if ("8-k" in n or "8k" in n or "ex99" in n or "ex-99" in n or "press" in n) else 1
...
# =========================
# iXBRL/HTML table parsing helpers
# =========================
...
# ---------- ETH table detection & mapping ----------
def _find_eth_header_row(rows: List[List[str]]) -> Optional[int]:
    for i in range(min(6, len(rows))):
        rowtxt = " ".join(rows[i]).lower()
        if ("eth" in rowtxt) or ("ethereum" in rowtxt):
            return i
    return None

def _map_header_indices_eth(header_row: List[str]) -> Dict[str,int]:
    idx: Dict[str,int] = {}
    seen_avg = 0
    for j, cell in enumerate(header_row):
        ...
        if not n:
            continue
        if ("ethacquired" in n) or ("ethpurchased" in n) or ("ethereumacquired" in n):
            idx["period_eth_acquired"] = j
        ...
        elif ("aggregateethholdings" in n) or ("totalethholdings" in n) or ("ethholdings" in n):
            idx["total_eth_holdings"] = j
        elif "averagepurchaseprice" in n or "avgpurchaseprice" in n:
            if seen_avg == 0:
...
def parse_eth_table_rows(rows: List[List[str]]) -> Optional[Dict[str, Any]]:
    h = _find_eth_header_row(rows)
    if h is None:
        return None
    header = rows[h]
    idx = _map_header_indices_eth(header)
    if not idx:
        return None
    ...
    if "period_eth_acquired" in idx:
        out["period_eth_acquired"] = _number_to_float(data[idx["period_eth_acquired"]])
    ...
    if "period_avg_purchase_price" in idx:
        out["period_avg_purchase_price_usd"] = (
            _money_to_float(data[idx["period_avg_purchase_price"]])
            or _number_to_float(data[idx["period_avg_purchase_price"]])
        )
    if "total_eth_holdings" in idx:
        out["total_eth_holdings"] = _number_to_float(data[idx["total_eth_holdings"]])
    ...
    if "total_avg_purchase_price" in idx:
        out["total_avg_purchase_price_usd"] = (
            _money_to_float(data[idx["total_avg_purchase_price"]])
            or _number_to_float(data[idx["total_avg_purchase_price"]])
        )

    for k in ("period_eth_acquired", "total_eth_holdings"):
...
def extract_eth_metrics_from_url(url: str) -> Dict[str, Any]:
    html = get(url).text
    ...
    for rows in tables:
        metrics = parse_eth_table_rows(rows)
        if metrics:
            break
...
# =========================
def main():
    ap = argparse.ArgumentParser(description="Build simple ETH timeline (ticker, date, purchase, total_holdings) from SEC iXBRL/HTML tables.")
    ap.add_argument("--ticker", default="BMNR", help="Ticker to scan (default: BMNR)")
    ...
    # Filter by accession number YEAR to avoid tz issues when comparing dates
    ...
    for _, row in subset.iterrows():
        ...
        for u in urls:
            try:
                rec = extract_eth_metrics_from_url(u)
            except Exception as e:
        ...
        if got:
            ...
            print(f"No ETH table found for filing {row.get('form')} {accession}")

    if not records:
        print("No ETH tables parsed.")
        return

    # ---- Build SIMPLE output: ticker, date, purchase, total_holdings ----
    df = pd.DataFrame(records)

    # date = filingDate only
    date_dt = pd.to_datetime(df.get("filingDate"), errors="coerce")
    df["date"] = date_dt.dt.strftime("%Y-%m-%d")

    # purchase / total_holdings from iXBRL metrics
    df["purchase"] = pd.to_numeric(df.get("period_eth_acquired"), errors="coerce")
    df["total_holdings"] = pd.to_numeric(df.get("total_eth_holdings"), errors="coerce")

    simple = df[["date", "purchase", "total_holdings"]].copy()
    simple.insert(0, "ticker", args.ticker)

    # De-dup to one row per date: prefer rows with a purchase value, else higher total_holdings
    simple["_has_purchase"] = simple["purchase"].notna().astype(int)
    simple["_purchase"] = simple["purchase"].fillna(-1)
    simple["_total"] = simple["total_holdings"].fillna(-1)

    simple = (
        simple.sort_values(
            by=["date", "_has_purchase", "_purchase", "_total"],
            ascending=[True, False, False, False]
        )
        .drop_duplicates(subset=["date"], keep="first")
        .drop(columns=["_has_purchase", "_purchase", "_total"])
        .reset_index(drop=True)
    )

    # Save CSV + NDJSON (simple schema)
    csv_path = outdir / f"{args.ticker.lower()}_eth_timeline_simple.csv"
    simple.to_csv(csv_path, index=False)
    print(f"Saved CSV -> {csv_path.resolve()}")

    ndjson_path = outdir / f"{args.ticker.lower()}_eth_timeline_simple.ndjson"
    with ndjson_path.open("w", encoding="utf-8") as f:
        for _, r in simple.iterrows():
            # ensure plain JSON-friendly types
            rec = {
                "ticker": r["ticker"],
                "date": r["date"],
                "purchase": (None if pd.isna(r["purchase"]) else float(r["purchase"])),
                "total_holdings": (None if pd.isna(r["total_holdings"]) else float(r["total_holdings"])),
            }
            f.write(json.dumps(rec, ensure_ascii=False) + "\n")
    print(f"Saved NDJSON -> {ndjson_path.resolve()}")

    # Console preview
    print("\nSimple ETH timeline (ticker, date, purchase, total_holdings):")
